...
class ContainerViewControllerTests : XCTestCase {
  ...
  func testStackEqualityVariadicInit() {
    ...
    // Instantiate a new container view controller and pass both VC's to
    // it's variadic init
  ...
  func testCanAnimateTransition() {
    XCTAssertFalse(
      containerViewController.canAnimateTransition(),
      """
      View controller stack is empty and therefore there \
      cannot be an animated transition.
      """
    )
    // Create a view controller stack of two VC's
    let viewControllers = [UIViewController(), UIViewController()]
    ...
    XCTAssertTrue(
      containerViewController.canAnimateTransition(),
      """
      View controller stack is not empty and therefore \
      an animated transition can occur.
      """
    )
  }
  ...
  func testSetEventDelivery() {
    ...
    containerViewController.rx
      .event
      .emit(onNext: {
        // Test the event for `viewControllers2`
        if $0.position == .start {
          XCTAssertTrue(
            $0.containerViewController.viewControllers != viewControllers2,
    ...
    XCTAssertTrue(
      containerViewController.viewControllers == viewControllers1,
